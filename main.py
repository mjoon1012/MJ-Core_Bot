...
async def shutdown_bot():
    ...
    try:
        ...
        try:
            ...
            print("[경고] bot.close() timeout → 강제 종료 진행")

    except Exception as e:
        print(f"[시스템] 종료 중 오류 발생: {e}")

    finally:
        print("[시스템] 프로세스 종료")
        os._exit(0)   # 🔥 이게 핵심 (sys.exit 아님)
...
def get_role_data(xp: int):
    if xp >= 400:
        return "부장", discord.Color.gold()
    elif xp >= 300:
        return "과장", discord.Color.from_rgb(255, 165, 0)
    elif xp >= 200:
        return "대리", discord.Color.green()
    elif xp >= 100:
        return "사원", discord.Color.blue()
    else:
        return "인턴", discord.Color.light_gray()
...
def create_info_embed(target: discord.Member, xp: int, level: int):
    ...
    embed.add_field(name="🎖️ 직급", value=role_name, inline=False)
    embed.add_field(
        name="진행 상황",
        value=f"{bar} ({current_level_xp}/100 XP)",
        inline=False
    )
    return embed
...
class InfoButtonView(discord.ui.View):
    ...
    async def get_info(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not row:
            await interaction.response.send_message("데이터 없음", ephemeral=False)
            return
...
async def check_role_upgrade(member: discord.Member, xp: int):
    ...
    all_role_ids = set(ROLE_IDS.values())
    roles_to_remove = [
        r for r in member.roles
        if r.id in all_role_ids and r.id != target_id
    ]
...
async def on_ready():
    database.init_db()
    await bot.tree.sync()
    print(f"{bot.user} v1.1.0 로그인 완료!")
...
async def 포인트(interaction: discord.Interaction,
    ...
    embed.add_field(name="결과", value=f"{role_name} / {new_xp} XP")
...
async def 정보(interaction: discord.Interaction,
    ...
    await interaction.response.send_message(
        embed=create_info_embed(target, row[0], row[1]),
        ephemeral=False
    )
...
async def 종료(interaction: discord.Interaction):
    await interaction.response.send_message("종료 중...", ephemeral=False)
    await shutdown_bot()
...
async def main():
    ...
    # 터미널 입력 리스너 시작
    input_task = loop.create_task(terminal_input_listener())

    # 시그널 핸들러 (Windows 환경 예외 처리)
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            loop.add_signal_handler(sig, lambda: asyncio.create_task(shutdown_bot()))
        except NotImplementedError:
            # Windows 등 일부 환경에서 발생할 수 있는 오류 방지
            pass
...
if __name__ == "__main__":
    try:
        ...
        # 이미 봇 내부에서 처리 중이므로 여기서 추가 작업 불필요
        pass
    except Exception as e:
